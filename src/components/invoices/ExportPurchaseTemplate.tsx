...
export default function ExportPurchaseTemplate({ entry }) {
  ...
  const exportPDF = async () => {
    const purchaseElement = document.getElementById("purchase-wrapper");
    if (!purchaseElement) return;

    const canvas = await html2canvas(purchaseElement, {
      scale: 3,
      useCORS: true,
    });
      ...
      <div id="purchase-wrapper" style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>Purchase No: {entry.purchase_no?.replace('PUR-', '')}</div>
            <div>GRN No: {entry.grn_no || 'N/A'}</div>
          </div>
          <div style={{
            display: 'flex',
            justifyContent: 'space-between',
          ...
          <table style={{
            ...
            <thead>
              <tr style={{
                ...
                <th style={{ border: '1px solid #000', padding: '10px', fontWeight: 'bold' }}>Products</th>
                <th style={{ border: '1px solid #000', padding: '10px', fontWeight: 'bold' }}>Quantity</th>
                <th style={{ border: '1px solid #000', padding: '10px', fontWeight: 'bold' }}>Weight<br/>Per Unit</th>
