...
export default function ProductReport() {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      </head><body><h1>Product Report</h1>${printContent.innerHTML}</body></html>
    `);
    printWindow.document.close();
    printWindow.print();
  };

  const totalWeight = rows.reduce((sum, r) => sum + Number(r.net_weight || 0), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      {rows.length > 0 && (
        <Card>
          <CardHeader className="py-3"><CardTitle className="text-lg">Product Entries ({rows.length})</CardTitle></CardHeader>
